...
**Why the manual path now?** The upstream `winddriver/Delphi-Cross-Socket` is actively maintained and changes frequently. A Boss-installable wrapper inevitably drifts behind upstream between fork syncs, and previously fork-only fixes (PATCH-IOCP-1, zero-body parser hang, `_OnBodyEnd` nil-guard, PATCH-CSHTTP-2, mTLS via `AddCACertificate(File)` + `SetVerifyPeer(Boolean)`, password-in-`SetPrivateKeyFile`) have all been merged into upstream as of 2026-Q2/Q3 — they no longer justify maintaining a fork as the default install target. The community fork ([github.com/freitasjca/Delphi-Cross-Socket](https://github.com/freitasjca/Delphi-Cross-Socket)) remains a **supported alternative** (release `v1.0.6`) for users who need `SetCipherList` (TLS 1.2 cipher override, `TLSOPT-2`) or the keep-alive retry in `TCrossHttpClient` (PATCH-CSHTTP-3), or prefer one fewer manual clone, with the explicit trade-off that the fork lags upstream commits between syncs.

The ideal long-term outcome is for the **original repository** to merge the two remaining fork-only additions (`SetCipherList`, PATCH-CSHTTP-3), after which the fork's only remaining differentiator (bundled CnPack convenience) becomes a much smaller value proposition.
