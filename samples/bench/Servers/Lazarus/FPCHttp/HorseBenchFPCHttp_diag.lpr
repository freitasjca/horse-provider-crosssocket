program HorseBenchFPCHttp_diag;

{
  Horse Performance Benchmark Server — FPC fphttpserver — DIAGNOSTIC (Lazarus)
  ==============================================================================

  Identical to HorseBenchFPCHttp.lpr (same routes, ports, middleware) but wraps
  the full pipeline in a diagnostic middleware that captures every exception that
  causes an HTTP 500, aggregated by class name.

  Log file: horse_500_diag_fpchttp.log  (written next to the executable)

  Run:
    ./HorseBenchFPCHttp_diag --middleware    ← listens on 9017, captures 500s
    bombardier -c 100 -n 200000 --http1 http://127.0.0.1:9017/ping

  Shutdown note:
    Ctrl-C terminates via the default SIGINT disposition. The diagnostic summary
    is printed in a try/finally block and WILL be lost on hard SIGINT. To ensure
    the summary is printed, send SIGTERM (kill -15 <pid>) and wait for a clean exit
    — or reduce the bombardier count and let the run finish naturally.

  Same search path requirements as HorseBenchFPCHttp.lpr, plus:
    ../../../../../horse/src  (for Horse.Exception.Interrupted)
}
...
begin
  ...
  LogLine('Compare "entered" vs bombardier total:');
  LogLine('  entered < total -> 500s generated BELOW Horse (fphttpserver bridge)');
...
begin
  ...
  try
    ...
    LogLine(Format('Diagnostic run start %s | port %d | %s | provider=FPCHttp',
    ...
    WriteLn(Format('[HorseBench/FPCHttp DIAG] Listening on http://127.0.0.1:%d  [%s]',
      [GPort, GModeLabel]));
    WriteLn(Format('[HorseBench/FPCHttp DIAG] Capturing 500 exceptions to: %s',
      [GLogFile]));
    WriteLn('Send SIGTERM (kill -15) for a clean shutdown with summary.');

    if GMaxConn > 0 then
    begin
      THorse.MaxConnections := GMaxConn;
      WriteLn(Format('MaxConnections override: %d (no effect on fphttpserver)', [GMaxConn]));
    end;
    ...
    WriteSummary;
    WriteLn('[HorseBench/FPCHttp DIAG] Stopped cleanly.');
  finally
    GErrorCounts.Free;
    GLogLock.Free;
  end;
