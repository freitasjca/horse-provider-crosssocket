...
{
  Horse Performance Benchmark Server — mORMot2 transport — DIAGNOSTIC (Lazarus)
  ===============================================================================

  Identical to HorseBenchMormot.lpr (same routes, ports, middleware) but wraps
  ...
  Log file: horse_500_diag_mormot.log  (written next to the executable)

  Run:
    ./HorseBenchMormot_diag --middleware    ← listens on 9013, captures 500s
    bombardier -c 100 -n 200000 --http1 http://127.0.0.1:9013/ping

  Same search path requirements as HorseBenchMormot.lpr.
}
...
begin
  ...
  LogLine('  entered < total -> 500s generated BELOW Horse (mORMot bridge)');
...
begin
  ...
  try
    ...
    LogLine(Format('Diagnostic run start %s | port %d | %s | provider=mORMot (Lazarus)',
    ...
    WriteLn(Format('[HorseBench/mORMot·Laz DIAG] Listening on http://127.0.0.1:%d  [%s]',
      [GPort, GModeLabel]));
    WriteLn(Format('[HorseBench/mORMot·Laz DIAG] Capturing 500 exceptions to: %s',
      [GLogFile]));
    ...
    begin
      ...
      WriteLn(Format('MaxConnections override: %d (no transport effect on mORMot)', [GMaxConn]));
    end;

    if GListenQueue > 0 then
      WriteLn(Format('ListenQueue %d ignored (mORMot-only flag)', [GListenQueue]));

    THorse.Listen(GPort);

    WriteSummary;
    WriteLn('[HorseBench/mORMot·Laz DIAG] Stopped cleanly.');
  finally
    GErrorCounts.Free;
    GLogLock.Free;
  end;
