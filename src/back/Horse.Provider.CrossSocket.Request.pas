...
begin
  ...
  if (ACrossReq.BodyType = btBinary) and Assigned(BodyObj) then
  begin
    Stream := BodyObj as TStream;
    if Stream.Size > 0 then
    begin
      Stream.Position := 0;
      // setter overload: function Body(const ABody: TObject): THorseRequest
      // confirmed in patched Horse.Request.pas
